...
export class TranslatorService {
    ...
    public initializeOpenAIClient() {
        ...
        if (!apiKey) {
            this.outputChannel.appendLine("❌ API key is not set in the vendor configuration");
            throw new Error("API key is not set in the vendor configuration");
        }
        if (!apiEndpoint) {
            this.outputChannel.appendLine("❌ API endpoint is not set in the vendor configuration");
            throw new Error("API endpoint is not set in the vendor configuration");
        }
        if (!model) {
            this.outputChannel.appendLine("❌ Model is not set in the vendor configuration");
    ...
    private async standardTranslateContent(
        ...
        const translatedContent = response.choices[0]?.message?.content || originalContent;
        ...
        for await (const chunk of stream) {
            ...
            if (content) {
                ...
                if (!foundNoNeedTranslate &&
                    ...
                    this.outputChannel.appendLine(`🔄 AI indicated no translation needed (in stream)`);
                ...
                if (!foundNoNeedTranslate) {
                    ...
                    if (!content.includes(AI_RETURN_CODE.NO_NEED_TRANSLATE) &&
                        ...
                        const fullCodeIndex = content.indexOf(AI_RETURN_CODE.NO_NEED_TRANSLATE);
                        const partialCodeIndex = content.indexOf(uuidFirstPart);

                        // Find the earliest occurrence of any part of the UUID
                        const indexToUse = (fullCodeIndex >= 0 && partialCodeIndex >= 0)
                            ? Math.min(fullCodeIndex, partialCodeIndex)
                            : (fullCodeIndex >= 0 ? fullCodeIndex : partialCodeIndex);
                        ...
                        foundNoNeedTranslate = true;
                        this.outputChannel.appendLine(`🔄 AI indicated no translation needed (in chunk)`);
